...
- Si tu détectes une question sur la réglementation locale (${p.region || 'France'}), sois précis`;
}
...
async function envoyerMessage(messageUtilisateur) {
  ...
  const reponse = await fetch('/api/chat', {
    ...
    body: JSON.stringify({ messages: historique, systemPrompt: getSystemPrompt() })
  });
...
function traiterReponse(contenu) {
  try {
    ...
    if (json.action === 'devis') return { type: 'devis', lignes: json.lignes };
  } catch (e) {
